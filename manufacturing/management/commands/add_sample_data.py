...
class Command(BaseCommand):
    help = 'Create sample data for the manufacturing application'
    ...
    def handle(self, *args, **kwargs):
        ...
        for station in workstations:
            if 'AMADA' in station.name:
                # High performance metrics for AMADA stations
                WorkstationEfficiencyMetric.objects.create(
                    workstation=station,
                    total_working_time=timedelta(hours=random.uniform(150, 200)),
                    total_idle_time=timedelta(hours=random.uniform(5, 20)),
                    total_material_used=random.uniform(1500, 2000),
                    total_material_wasted=random.uniform(20, 50),
                    total_items_processed=random.randint(400, 500),
                    total_items_with_defects=random.randint(5, 15)
                )
            else:
                # Normal performance metrics for other stations
                WorkstationEfficiencyMetric.objects.create(
                    workstation=station,
